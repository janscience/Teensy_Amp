...
temperature compensated real-time clock to the PCB. Or just provide
pins for existing PCBs with this chip. Note that the default
implementation works only for the first I2C bus.

An interesting alternative for the real-time clock is the
[RV-3028-C7](https://www.microcrystal.com/en/products/real-time-clock-rtc-modules/rv-3028-c7/). It
is small, precise, but not temperature compensated.

Using the onboard real-time clock requires power via a coin battery.
Place a coin-battery holder (CR 2032) on the PCB and connect the
battery to the VBAT pin:
...
This turns out to be not a good idea, because the second SD card draws
too much power.
