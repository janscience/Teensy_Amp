...
The 4 signals come in via ERNI MiniBridge right angle male with 4 contacts.
...
Two ERNI MiniBridge vertical males with 6 contacts (external GND, external 3.3V, CANL CANH, I/O, unueed) one for upstream, one for downstream.
...
### Tests regarding CAN bus

- Common power supply results in non-useable noisy signals.
- A powerbank for each PCB, and two boards with connected GND (or via 1k) works.
- Test CAN bus with individual power supplies.


## Real-time clock

Any real-time clock needs power via a coin battery.
Place a coin-battery holder (CR 2032) on the PCB.

For powering the Teensy onboard real-time clock, connect the battery
to the VBAT pin:
...
Alternatively: add a better real-time clock to the PCB:

- [DS3231](https://www.analog.com/media/en/technical-documentation/data-sheets/ds3231.pdf): temperature compensated. Default implementation only for first I2C bus.
- [RV-3028-C7](https://www.microcrystal.com/en/products/real-time-clock-rtc-modules/rv-3028-c7/): small, precise, but not temperature compensated.
...
Add two USB C power inputs with on/off switch.

Add diodes to allow for changes in power supply on the fly.

Add alternative power input via isolated DC/DC converter from CAN bus
power supply. Maybe for 3 - 12V.


### Tests

- Common power supply results in noisy, non-usable signals
- Common power supply with resistances between PCBs does not work because of voltage drops.
- Common power supply with individual non-isolated DC/DC converters results in noisy signals.
- Common power supply with individual isolated DC/DC converters results null signals.
- Power supply on single PCB over isolated DC/DC converter results in null signal (over water or direct input).
- Power supply on single PCB over isolated DC/DC converter with GND cable from battery ground results in null signal.
- Power supply on single PCB over isolated DC/DC converter with GND cable from isolator V- output results in null signal.

Further tests:

- Connect battery GND with isolator V- by 100k resistance etc.
...
This chip is amazing. Can we deal with this packaging?
...
- I2C: ERNI MiniBridge vertical male with 6 contacts (GND, 3.3V, SCL, SDA. LED1, LED2)
- SPI: ERNI MiniBridge vertical male with 6 contacts (GND, 3.3V, CS, SCK, MOSI, MISO)
- OneWire: 3 pins in 2.54mm spacing (GND, 3.3V, DATA + 4.7kOhm to 3.3V)
...
- Add better real-time clock (e.g. [RV-3028-C7](https://www.microcrystal.com/en/products/real-time-clock-rtc-modules/rv-3028-c7/)
- Replace signal screw-terminals by ERNI MiniBridge (right angle male SMT type A with 4 contacts on PCB, Female with 180 cable outlet, Koshiri variant, see https://www.mouser.de/datasheet/2/418/9/ENG_DS_erni_minibridge_1_27mm_connectors_ds_2206-3355850.pdf).
